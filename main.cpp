...
const int INSUFFICIENT_MIN_OR_EXCEED_PW_LENGTH = 10;
const int INSUFFICIENT_MIN_OR_EXCEED_MKEY_LENGTH = 20;
const int CONTAINS_SPACE = 30;
const int INSUFFICIENT_UPPERCASE = 40;
const int INSUFFICIENT_LOWERCASE = 50;
...
*/
void showError(int danger, std::string errReason, sqlite3* db = nullptr, int httpRTN = -1, int policyTriggerer = -1, httplib::Error httpErr = httplib::Error::Success) {
    ...
    if (danger == SYSTEM_ERROR) {
        if (errReason == "SQL") {
            ...
            msg += tmp;
        } else if (errReason == "HTTP"){
            msg += "system - HTTP ERROR: " + std::to_string(static_cast<int>(httpErr));
        } else {
        ...
        if (errReason == "PASSWORDPOLICY"){
            ...
            switch (policyTriggerer) {
                case INSUFFICIENT_MIN_OR_EXCEED_PW_LENGTH:
                    msg += "Your password is either too small or too long";
                    break;
                case INSUFFICIENT_MIN_OR_EXCEED_MKEY_LENGTH:
                    msg += "Your master key is either too small or too long";
                    break;
                case CONTAINS_SPACE:
            ...
            msg += "unknown - RETURN STATUS: "+ std::to_string(httpRTN);
        }
    }

    if (errReason == "HTTP") {
        if (danger == SYSTEM_ERROR) {
            msg += "\nPLEASE REPORT THE EXACT ERROR MESSAGE ON help.simplepassman@gmail.com\n";
        }
        msg += "\nPLEASE REPORT THE REPORT STATUS ON help.simplepassman@gmail.com\n";
    }

    if (danger == SYSTEM_ERROR && !errReason.empty() && errReason != "HTTP") {
...
bool checkForUpdate(){
    httplib::Client cli("https://github.com");cli.set_follow_location(true);
    const auto r = cli.Get("/"+repoOwner+"/"+repoName+"/releases/latest/");
    if (!r || r->status != 200) {
        if (r) {
            // Must be response code thats causing the error, thus show the error message
            showError(USER_ERROR, "HTTP", nullptr, r->status);
            return false;
        }
        // R must not exist, thus print the error of r
        showError(SYSTEM_ERROR, "HTTP", nullptr, -1, -1, r.error());
        return false;
    }
    std::string latest = r->location.substr(r->location.rfind('/') + 1);
    if (currV != latest) {
        // User is not on the latest version
        char response = 'n';
        std::cout << "A new version was detected ( " + latest + " ). Would you like to update? (y/n): ";
        std::cin >> response;
        if (response == 'y' || response == 'Y') {
            std::cout << "To update:\n"
                    "1. Visit the following URL: " << r->location << "\n"
                    "2. Download the version for your operating system.\n"
                    "3. Replace your current executable with the downloaded one.\n"
                    "**DO NOT DELETE THE FILE data.db IF YOU WISH TO KEEP YOUR DATA.**\n"
                    "If you change your mind you may reopen this program.";
            exit(0);
        } else {
            std::cout << "Continuing on current version..";
            return true;
        }
    }
    return true;
